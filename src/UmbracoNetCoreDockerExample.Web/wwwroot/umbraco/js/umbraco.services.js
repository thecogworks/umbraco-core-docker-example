       ...
       */
                getAvailableAliasesForBlockContent: function getAvailableAliasesForBlockContent() {
                    ...
                    return this.blockConfigurations.map(function (blockConfiguration) {
                        return _this.getScaffoldFromKey(blockConfiguration.contentElementTypeKey).contentTypeAlias;
                    });
                },
                /**
       * @ngdoc method
  ...
  */
        service.copy = function (type, alias, data, displayLabel, displayIcon, uniqueKey, firstLevelClearupMethod) {
            ...
            var entry = {
                ...
                type: type,
                alias: alias,
                data: prepareEntryForStorage(data, firstLevelClearupMethod),
                label: displayLabel,
                icon: displayIcon,
                date: Date.now()
            };
  ...
  */
        service.copyArray = function (type, aliases, datas, displayLabel, displayIcon, uniqueKey, firstLevelClearupMethod) {
            ...
            var copiedDatas = datas.map(function (data) {
                return prepareEntryForStorage(data, firstLevelClearupMethod);
            });
            // remove previous copies of this entry:
            storage.entries = storage.entries.filter(function (entry) {
                return entry.unique !== uniqueKey;
...
**/
     ...
     *  }
     */
            getProcessedImageUrl: function getProcessedImageUrl(imagePath, options) {
                if (!options) {
                    return imagePath;
                }
                return umbRequestHelper.resourcePromise($http.get(umbRequestHelper.getApiUrl('imagesApiBaseUrl', 'GetProcessedImageUrl', {
                    imagePath: imagePath,
      ...
      */
            getTreeTemplateUrl: function getTreeTemplateUrl(treeAlias, action) {
                ...
                if (packageTreeFolder) {
                    return (Umbraco.Sys.ServerVariables.umbracoSettings.appPluginsPath + '/' + packageTreeFolder + '/backoffice/' + treeAlias + '/' + action + '.html').toLowerCase();
                } else {
 ...
 */
    function tinyMceService($rootScope, $q, imageHelper, $locale, $http, $timeout, stylesheetResource, macroResource, macroService, $routeParams, umbRequestHelper, angularHelper, userService, editorService, entityResource, eventsService, localStorageService, mediaHelper) {
